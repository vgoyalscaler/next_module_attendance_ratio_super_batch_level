...
def train_target_encodings(df):
    ...
    df['module_name'] = df['module_name'].map(module_name_target_encoded).fillna(overall_y_mean)
...
def main():
    st.title("Predicting next module live attendance ratio at super_batch level")
    
    # take csv file as input 
    uploaded_csv = st.file_uploader("Upload the csv file of the required batches and corresponding module_names selected from: https://docs.google.com/spreadsheets/d/1gCImAFX1TVeKCSRDCZrIcJWWpKSbAZaFwljqVlWQgxs/edit?gid=0#gid=0", type=['csv'])
    
    if uploaded_csv:
        df = pd.read_csv(uploaded_csv)
        df = df[['super_batch_name', 'batch_module_order', 'instructor_email', 'module_name', 
...
# !jupyter nbconvert --to script next_module_attendance_ratio_super_batch_level.ipynb
...
# packages_list = [f"{package.project_name}=={package.version}" for package in installed_packages]
